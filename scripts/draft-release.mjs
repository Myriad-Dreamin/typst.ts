...
const main = async () => {
  ...
  const changelogPlainRaw = await run('parse-changelog ./CHANGELOG/CHANGELOG-0.8.md');
  // **Full Changelog**:
  // Patch the full changelog link
  const fullChangelogLine =
    /\*\*Full Changelog\*\*: https:\/\/github.com\/Myriad-Dreamin\/typst\.ts\/compare\/v(\d+\.\d+\.\d+)...v(\d+\.\d+\.\d+)/;
  ...
  const changelogPlain = changelogPlainRaw.replace(fullChangelogLine, (_match, p1, p2) => {
    ...
    return `\*\*Full Changelog\*\*: https://github.com/Myriad-Dreamin/typst.ts/compare/v${p1}...v${versionToUpload}`;
  });
  if (!anyMatched) {
