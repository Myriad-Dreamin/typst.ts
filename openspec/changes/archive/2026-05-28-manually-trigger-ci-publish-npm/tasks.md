...
- [x] 2.1 Add a manual GitHub Actions workflow that verifies the publishable subprojects under `projects/` as one release batch, including `projects/rustdoc-typst-demo`.
- [x] 2.2 Run the required project build steps and dry-run publish, pack, or Cargo verification commands in that workflow.
- [x] 2.3 Fail the workflow on any covered project verification error and summarize the verified project set.
...
- [x] 5.1 Create a manual project release workflow for `hexo-renderer-typst`, `@myriaddreamin/rehype-typst`, `@myriaddreamin/vite-plugin-typst`, `@myriaddreamin/highlighter-typst`, and `projects/rustdoc-typst-demo` as one release batch.
- [x] 5.2 Gate live project publishing on successful package verification, project verification, and package publishing.
- [x] 5.3 Add project-specific build steps so `@myriaddreamin/highlighter-typst` and `@myriaddreamin/vite-plugin-typst` build before publish.
- [x] 5.4 Use project-specific publish commands so `projects/rustdoc-typst-demo` publishes through Cargo while the npm projects use their npm publish paths.
- [x] 5.5 Exclude unpublished or private projects such as `@myriaddreamin/vistyp` from the release batch.
...
- [x] 7.1 Document which packages, container artifacts, and Rust publish targets are covered by each workflow.
- [x] 7.2 Record the npm trusted publisher registration steps for maintainers, including the workflow filenames and environment expectations.
- [x] 7.3 Document the fixed workflow order and which credentials and approval boundaries apply to npm, GHCR, GitHub Release uploads, and crates.io.
