...
impl GlyphRenderTask {
    pub fn render_glyph(&mut self, glyph_id: &str, glyph_item: &GlyphItem) -> Option<String> {
    ...
    fn render_glyph_pure_inner(glyph_id: &str, glyph_item: &GlyphItem) -> Option<String> {
        match glyph_item {
            GlyphItem::Image(image_glyph) => Self::render_image_glyph(glyph_id, image_glyph),
    ...
    fn render_outline_glyph(glyph_id: &str, outline_glyph: &OutlineGlyphItem) -> Option<String> {
        let symbol_def = format!(
